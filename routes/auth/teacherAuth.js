            ...
            user.save().then(() => {
              let mailDetails = {
                ...
                html: `
                          ...
                          <h5>click in this <a href="https://attendence-portal.herokuapp.com/teacher/activate/${token}">link</a> to activate account.</h5>
                          `,
              };
...
router.post("/forgot-password", async (request, response) => {
  ...
  crypto.randomBytes(32, (error, buffer) => {
    ...
    const token = buffer.toString("hex");
    Teacher.findOne({ email: email }).then(async (user) => {
      if (!user) {
...
router.post("/new-password", async (request, response) => {
  ...
  Teacher.findOne({ resetToken: token, expireToken: { $gt: Date.now() } })
    .then((user) => {
      if (!user) {
...
router.get("/activate/:token", async (request, response) => {
  ...
  Teacher.findOne({ activationToken: token }).then((user) => {
    if (user) {
